...
class OpenAICompatibleLLMClient:
    ...
    def generate_readme(self, project: ProjectInfo) -> str:
        try:
            ...
            raise RuntimeError("未安装 openai 依赖，请先执行 `pip install -e '.[openai]'` 或 `pip install openai`") from exc
...
class ConfiguredLLMClient:
    config: DocPilotConfig
    fallback: TemplateLLMClient

    def generate_readme(self, project: ProjectInfo) -> str:
        provider = _resolve_provider(self.config)
        if provider == "openai":
        ...
        return self.fallback.generate_readme(project)


def create_llm_client(config: DocPilotConfig) -> LLMClient:
    return ConfiguredLLMClient(config=config, fallback=TemplateLLMClient(config))


def _resolve_provider(config: DocPilotConfig) -> str:
