...
def _build_config(provider: Optional[str]) -> DocPilotConfig:
    return DocPilotConfig(
        llm_provider=provider or DEFAULT_CONFIG.llm_provider,
        ...
        deepseek_base_url=os.getenv("DEEPSEEK_BASE_URL", DEFAULT_CONFIG.deepseek_base_url),
    )
...
@click.option("--provider", type=click.Choice(["template", "openai", "deepseek"]), default=None, help="指定 README 生成器；不传时会自动优先使用已配置的大模型")
@click.option("--msg", default="docs: update README", show_default=True, help="启用提交时使用的 Git 提交信息")
@click.option("--commit", is_flag=True, default=False, help="生成后自动提交当前改动")
@click.option("--auto-push", is_flag=True, default=False, help="提交后自动 push")
def generate(template: Optional[Path], provider: Optional[str], msg: str, commit: bool, auto_push: bool) -> None:
    ...
    if commit:
        try:
            ...
            raise DocPilotError(f"README 已生成但提交失败：{exc}") from exc
        click.echo(f"README 已生成并提交：{path}")
        return
...
@main.command()
@click.option("--msg", default="docs: update README", show_default=True, help="Git 提交信息")
@click.option("--template", type=click.Path(exists=True, dir_okay=False, path_type=Path), default=None, help="使用本地模板文件直接生成 README")
@click.option("--provider", type=click.Choice(["template", "openai", "deepseek"]), default=None, help="指定 README 生成器；不传时会自动优先使用已配置的大模型")
@click.option("--auto-push", is_flag=True, default=False, help="提交后自动 push")
def run(msg: str, template: Optional[Path], provider: Optional[str], auto_push: bool) -> None:
    ...
    try:
        _commit_and_maybe_push(msg, False)
    except subprocess.CalledProcessError as exc:
        raise DocPilotError(f"DocPilot 流程失败并已尝试回滚：{exc}") from exc
    if auto_push:
        if click.confirm("是否现在执行 push？", default=False):
            try:
                _run_git_command(["git", "push"])
            except subprocess.CalledProcessError as exc:
                raise DocPilotError(f"push 失败：{exc}") from exc
        else:
            click.echo("已跳过 push")
